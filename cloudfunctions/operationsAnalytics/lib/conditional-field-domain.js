const { normalizeOptionLinkageInput, buildOptionLinkageContext } = require('./option-linkage-domain')
...
function cloneDefinition(field) {
  ...
  if (hasOwn(field, 'optionLinkage')) result.optionLinkage = normalizeOptionLinkageInput(field.optionLinkage)
  return result
}

function normalizeConditionalFields(fields) {
  buildOptionLinkageContext(fields)
...
function resolveConditionalFields(fields, submitted) {
  ...
  const linkage = buildOptionLinkageContext(normalizedFields)
  ...
  for (const field of normalizedFields) {
    const visible = linkage.members.has(field.fieldKey)
      ? linkage.project(field, valuesByKey) : visibleDefinition(field, valuesByKey, visibleKeys)
    if (!visible) continue
...
function conditionalFieldDigestProjection(fields) {
  return normalizeConditionalFields(fields).map(field => ({
    ...
    ...(hasOwn(field, 'optionLinkage') ? { optionLinkage: normalizeOptionLinkageInput(field.optionLinkage) } : {})
  }))
}
