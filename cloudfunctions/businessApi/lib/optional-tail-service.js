...
function decisionTiming(calculated, at) {
  if (calculated && calculated.status === 'calculated' &&
    ...
    return {
      decisionAt: new Date(at),
      decisionTimingStatus: 'calculated',
      decisionWorkMinutes: calculated.minutes,
      decisionCalendarVersion: calculated.calendarVersion || null
    }
  }
  if (calculated && calculated.status === 'pending_calendar' && calculated.minutes === null) {
    return {
      decisionAt: new Date(at),
      decisionTimingStatus: 'pending_calendar',
      decisionWorkMinutes: null,
...
function createOptionalTailService({
  ...
  async function decide({ actor, input }) {
    ...
    const inputHash = sha256(JSON.stringify([
      ...
      safeInput.expectedLineVersion, safeInput.expectedNodeVersion,
    ...
    if (normalized.decision === 'activate') {
      const processingMinutes = Number(context.processingSlaWorkHours) * 60
      if (!Number.isSafeInteger(processingMinutes) || processingMinutes <= 0) {
        throw createError('VERSION_CONFLICT')
      }
      const due = await workTimeService.tryAddWorkMinutes(new Date(at), processingMinutes)
      Object.assign(timing, processingTiming(due, at))
    }
...
module.exports = { createOptionalTailService }
