...
const { fieldError } = require('./operations-field-query')
const PURPOSE = 'operations-field-analysis-v1'
const HEX = /^[a-f0-9]{64}$/
...
function createOperationsFieldAnalysisCursor({ secret, clock = () => new Date() }) {
  ...
  return {
    encode(body) {
      ...
      return 'ofa1.' + Buffer.concat([iv, cipher.getAuthTag(), data]).toString('base64url')
    },
    decode(token, binding) {
      let body
      try {
        if (typeof token !== 'string' || !/^ofa1\.[A-Za-z0-9_-]{40,2000}$/.test(token)) throw new Error('format')
        const packed = Buffer.from(token.slice(5), 'base64url')
        if (packed.toString('base64url') !== token.slice(5)) throw new Error('canonical')
        const decipher = crypto.createDecipheriv('aes-256-gcm', key, packed.subarray(0, 12))
        decipher.setAAD(Buffer.from(PURPOSE)); decipher.setAuthTag(packed.subarray(12, 28))
...
module.exports = { createOperationsFieldAnalysisCursor }
