...
test('概览从服务端审核与通知分页计算真实数量并提供审核和消息入口', async () => {
  ...
  const cloud = {
    callBusinessApi: async (action, payload) => {
      ...
      if (action === 'listBusinessLines') return { items: [{ _id: 'line-1', status: 'active' }, { _id: 'line-2', status: 'completed' }] }
  ...
  assert.deepEqual(result.stats, {
    active: 1, pendingMine: null, pendingMineAvailable: false, pendingReviews: 2, unreadNotifications: 1, completed: 1
  })
