...
若第 2–4 步任一步失败，停止迁移、保持旧索引和旧部署，并从已导出的副本恢复到迁移前的**一致**状态。移除旧索引或启用新云函数后，不要仅恢复其中一个集合：必须一起恢复用户 OpenID、绑定记录和守卫文档，随后重新执行脱敏一致性核对。不得通过删除最后一个活动超级管理员来“修复”计数。

## 恢复码哈希和守卫配置

使用密码管理器生成一次性恢复码。通过获批准的、不会把明文写入 shell 历史、终端输出、剪贴板同步或跟踪文件的本地操作流程计算 SHA-256。原始恢复码只在初始化或恢复调用时由操作员本地输入，成功后立即弃用；不要把它放进环境变量、脚本参数或 JSON 文件。
