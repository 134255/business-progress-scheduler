...
| `node_review_rounds` | `nodeId` 升序、`reviewRoundNumber` 升序 | 否 | 节点审核历史：固定nodeId、轮次倒序反向扫描与小于轮次游标 |
...
5. 小程序使用仓库固定的 `cos-wx-sdk-v5@1.8.0` 高级 `uploadFile`：SDK 自动选择简单上传或分块上传，页面最多并发 3 个文件，并只对可重试网络/5xx/429 错误退避重试。单份上传凭据有效 15 分钟；进入到期前两分钟或 COS 返回明确凭据过期错误时，客户端只可调用受保护的 `refreshEvidenceUploadAuthorization` 刷新原 `evidenceId`、原对象键和原预约令牌的短期授权。服务端必须重新核对当前账号、节点、版本、轮次、处理角色、格式策略和 24 小时孤立期限；客户端并发刷新单飞、单文件最多刷新八次且进度不得倒退。不得复用过期凭据、另建凭证预约或切换对象键。
...
#### 审核意见与历史增量修复验收（2026-09-20）

先发布 `businessApi`，再上传并验证小程序新版本；仅发布云函数无法补出旧客户端缺少的展示。复用既有轮次和投票索引，不改集合权限、定时器或历史数据。无操作动作的云函数启动测试不能代替业务验收。

- 在有权成员账号下只读打开已有节点记录，核对审核历史中的轮次、审批人、时间、通过/驳回和意见；历史正文与原票一致，换行保留，空意见明确显示“未填写审核意见”。
- 核对审核详情中的每张已投票记录；节点返工进入新轮或已经完成后，旧轮仍可在节点记录页查看。取消/关闭只冻结售后头时，也能查看此前已投的意见，不出现新的审批按钮。
- 使用经批准的隔离数据检查超过5轮的“加载更多”、请求失败重试和切换账号；权限失效后已显示的审核历史应清空，迟到响应不得恢复。历史加载失败不阻断处理表单、不得覆盖未保存草稿。
- 不为验收改写生产原票或创建未授权测试售后。当前环境真实索引查询和手机渲染须独立验证；本地测试通过不等同于已上线。
...
| 慢速上传跨凭据期限 | 对 90–120 MiB 无敏感视频限速，使上传持续超过 15 分钟；同时观察 `evidences` 数量、对象键与页面进度 | 原凭据到期前后自动刷新同一预约的短期授权并继续分块；`evidenceId`、对象键和记录数不变，进度不倒退；完成后只生成一条 `available` 元数据 | 未验证 |
