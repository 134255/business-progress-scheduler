...
For ordinary feedback evidence, completed-line `retentionStartedAt` and `purgeDueAt` are authoritative for the whole business line. Attached evidence records declare `retentionScope: business_line` with `retentionSource: node_feedback`; future Task 9 amendment evidence may explicitly use `retentionScope: evidence` with `retentionSource: audit_amendment` and a later evidence-owned deadline. Unknown or inconsistent scope/source metadata fails closed. Task 11 must scan due completed lines and purge every associated evidence object by `businessLineId` in bounded chunks; earlier revisions therefore inherit the same deadline as the final revision.
