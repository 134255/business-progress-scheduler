...
- 模板最多允许一个 `activationMode: optional_tail`，且必须是最后一个节点并保留至少一名可解析的处理人。售后创建事务与必经节点一起预生成该节点的稳定编号、字段、处理人、审核人和显示名快照，但初始状态为 `awaiting_decision`，不能提前填写或上传。
...
- `decisionStartedAt` 到 `decidedAt` 是独立决定区间，不计入节点处理时长。日历缺失时保存 `pending_calendar` 和不可变边界，由 `calendarSync` 使用独立固定游标有界补算；运营统计仅把 `calculated` 决定时长作为分钟事实，同时为开启/跳过保存独立事件事实，避免把零分钟与缺失混淆。
- 待决定节点进入候选处理人的待办、通知和逐工作小时提醒，决定成功后立即停止该类提醒；开启后转入普通处理提醒。休眠或跳过的空追加节点不进入检索或分享，已开启且完成的节点沿用当前有效快照检索和固定分享。
...
- 需要两条 `business_nodes` 组合索引：`decisionTimingStatus ASC, _id ASC` 和 `optionalTailState ASC, nextDecisionReminderWorkHour ASC, decisionStartedAt ASC, _id ASC`。索引创建、函数部署和隔离数据验收属于部署状态，不能由本地测试替代。
