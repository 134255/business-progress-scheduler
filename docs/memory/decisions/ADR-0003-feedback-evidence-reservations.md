...
- Mark ordinary attached evidence with the explicit pair `retentionScope: business_line` and `retentionSource: node_feedback`; access and history derive its effective deadline from the strict line `purgeDueAt`. Reserve `retentionScope: evidence` plus `retentionSource: audit_amendment` for Task 9 corrections whose strict evidence-owned deadline may outlive the line. Unknown, mismatched, or malformed retention metadata fails closed. Unattached uploads remain governed by their orphan lease.
