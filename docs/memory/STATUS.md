...
- Task 8 of the template/node/field plan is implemented on its isolated worktree: protected `submitFeedback` and `getNodeHistory` routes accept only the trusted account actor, and the legacy feedback write/history handlers are no longer exposed. Active account-ID node assignees may append immutable typed revisions for `in_progress`, `blocked`, or `completed`; managers and members retain authorized history reads, while account/mixed relationship presence always selects fail-closed account authorization. Dynamic values are revalidated from the immutable node snapshot, and stored feedback preserves field key/name/type/value snapshots, node name/code, revision, account actor, timestamps, comment, status, exact aggregate evidence bytes, and safe request hashes without raw request keys or an unbounded evidence-ID manifest. Evidence attachment has no business file-count cap: a hidden deterministic reservation and a bounded 40-document claim cursor keep every transaction below CloudBase's 100-operation boundary, while exact total bytes remain capped at 20 MB. Prefix digests, fixed-ID revalidation, ownership, availability, expiry, relationship, and attachment checks make claims resumable and exclusive. A final transaction publishes one revision, advances exactly one next node or completes/freezes the line, assigns a common 60-calendar-day retention deadline from the recorded line-completion instant, and writes one deterministic audit. Genuine overlapping OR-sign completions produce one winner and one `NODE_ALREADY_COMPLETED` loser without duplicate revision, attachment, activation, or audit. Failed claims are hidden, eagerly aborted, and release evidence/node claims; expired reservations expose an idempotent recovery operation for Task 11, whose orphan cleanup must protect live claims and recover expired ones first. Physical CloudBase request/document byte limits remain platform constraints, not an application evidence-count rule. The durable protocol is recorded in `ADR-0003`.
...
1. Execute Task 8 from `docs/superpowers/plans/2026-08-07-template-node-fields.md`: persist immutable feedback revisions and atomically attach registered evidence.
2. Continue the remaining rejection, freeze, amendment, client, and evidence-retention tasks in plan order from the isolated `codex/` worktree.
