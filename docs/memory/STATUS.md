...
- Task 5 of the template/node/field plan is implemented on its isolated worktree: `createBusinessFromTemplate` accepts only authenticated, validated template-backed metadata; business codes use the Asia/Shanghai day and expand beyond four digits; immutable node codes expand beyond three digits; and a deterministic actor/request reservation makes retries idempotent without storing the raw request key. The CloudBase repository revalidates the enabled template version and every active assignee in a bounded fixed-document transaction that atomically reserves the daily sequence and writes an invisible `creating` line plus all snapshot nodes. A second bounded transaction verifies every deterministic node, publishes the line as `active`, and writes one secret-free audit record. Snapshots preserve the source template/version, copied field definitions, creator manager/member membership, all assignees, and ready/waiting node state. Unique-index collisions retry with a higher non-wrapping sequence, and operation-budget overflow fails closed with the shared maximum-48 contract. Task 6 (ordinary template-backed business creation UI) is next.
...
1. Execute Task 6 from `docs/superpowers/plans/2026-08-07-template-node-fields.md`: build ordinary template selection and template-backed business creation UI.
