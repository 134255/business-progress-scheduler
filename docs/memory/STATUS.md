...
- Task 8 of the template/node/field plan is implemented on its isolated worktree; formal-review round two fixes are locally verified but remain pending reviewer acceptance. Exact retries use a request fingerprint and transactionally revalidate the current active account/relationships before returning the deterministic published result, including publication races in the claim and finalization paths after next-node or final-line completion; concurrent identical public requests resolve the same result, changed payloads conflict, and disabled actors remain denied. Active account-ID assignees append immutable typed revisions, while history binds new evidence to the exact safe feedback revision and excludes missing, stale, or cross-revision associations; the legacy `evidenceIds` adapter remains isolated and redacts raw identity. Evidence attachment has no business count cap: 40-document claims stay below 100 operations and a digest binds cursor count, bytes, and ordered IDs. OR contention distinguishes completed winners, non-completion conflicts, live retryable leases, and recoverable expired/aborted claims. Only final-node completion starts retention. Ordinary attachment records the explicit `business_line/node_feedback` scope/source pair and access/history derive the strict line deadline for every revision; future amendments may opt into `evidence/audit_amendment` and a later strict evidence deadline. Unknown scopes and malformed line/evidence timestamps deny access before temporary-URL issuance, while unattached uploads retain their 24-hour orphan lease. Task 11 must recover expired reservations, clear node claims whose reservation is missing, then scan due completed lines and process all evidence by `businessLineId` in bounded chunks. Physical CloudBase request/document byte limits remain platform constraints. The durable protocol is recorded in `ADR-0003`.
