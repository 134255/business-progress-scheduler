...
Status captured: 2026-08-08 (Asia/Shanghai)

## Verified state

- Task 8 of the template/node/field plan is implemented and review-hardened on its isolated worktree. Exact retries use a request fingerprint and transactionally revalidate the current active account/relationships before returning the deterministic published result, even after the line advances or freezes; changed payloads conflict and disabled actors remain denied. The protected request requires a plain own-property object with every exact field. Active account-ID assignees append immutable typed `in_progress`, `blocked`, or `completed` revisions, while current account binding and schema precedence fail closed for history. Legacy history safely adapts legacy `evidenceIds`, strips raw OpenIDs, validates line/node/attachment associations, paginates all records, and applies deterministic timestamp/revision/ID ordering. Evidence attachment has no business count cap: 40-document claims stay below 100 operations and a digest binds cursor count, bytes, and ordered IDs; malformed cursors, zero-byte evidence, unsafe counters, and malformed stored deadlines fail closed. OR contention reports `NODE_ALREADY_COMPLETED` only when a published completed winner plus node/line state proves completion; non-completion winners conflict, live leases return retryable `FEEDBACK_COMMIT_IN_PROGRESS`, and expired, malformed, or aborted leases recover without stale transition timestamps. Only final-node completion starts retention: the line atomically owns the 60-calendar-day `retentionStartedAt`/`purgeDueAt`, evidence rows are not prematurely stamped, and history derives the same effective line deadline for earlier and final revisions. Task 11 must recover expired reservations before orphan deletion, then scan due completed lines and process all evidence by `businessLineId` in bounded chunks; Task 9 amendment evidence may later carry a newer explicit deadline. Physical CloudBase request/document byte limits remain platform constraints. The durable protocol is recorded in `ADR-0003`.
...
1. Execute Task 9 from `docs/superpowers/plans/2026-08-07-template-node-fields.md`: add previous-node rejection, frozen-state enforcement, and audited amendments.
2. Continue the remaining client and evidence-retention tasks in plan order from the isolated `codex/` worktree.
