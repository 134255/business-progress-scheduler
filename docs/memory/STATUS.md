...
- 2026-09-24 用户“全部执行”明确授权发布B1、上传包含A诊断的小程序及将历史/优化代码分批提交GitHub。已完成businessApi官方CLI inc-deploy（6498文件/9.0MB）及独立回读，6498/6498 SHA256与验收载荷一致、0差异；仅两个仓储内容变化，清单摘要9b23b2ca785dc9bbe71ffbd3164be35b62a76e7e02a7ed91dfd844d566ffbcb7。新鲜下载的发布前基线无额外差异；未部署operationsAnalytics或修改依赖、配置、权限、索引、Timer、业务数据。
  ...
  本轮新鲜验收：实际部署载荷QA businessApi1423/1423、客户端648/648、容量工具136/136、operationsAnalytics66/66，官方WXML10/6/8及结构4全部通过，0失败/取消/跳过。GitHub fetch成功，远端无新增（origin/main相对本地0/4）；按83路径历史功能与26路径A/B1优化分批集成，共108个不同路径，共享仓储只在索引使用发布前版本拆分，工作区未回退。附件、outputs、依赖、pycache不纳入Git。历史暂存树导出后76个API应用文件与发布前代码、178个客户端/配置与A前基线内容一致（仅行尾规范化）；独立客户端648/648通过。历史API初跑因导出树不含wx-server-sdk依赖失败，按package-lock对应线上依赖补齐重跑1422/1422通过，不改代码解决验证环境问题。

  GitHub集成完成：历史功能提交faaed643cc3fcc8ab96ed9efc981db6a4c079b3e，A/B1优化提交6c545b2e4016f3072034b1d0f50e338587e5ae4e；连同先前四个本地文档提交正常推送origin/main，未强推。git ls-remote实证远端main为6c545b2e4016f3072034b1d0f50e338587e5ae4e，与当时本地HEAD一致。优化暂存树独立回归再次1423后端/648客户端/136工具/4结构全通过，26文件与工作区一致，76应用文件与云端回读、178客户端/配置与实际上传源一致（行尾规范化）。两个提交前均核对Git树摘要与验收树完全相同；高置信密钥规则扫描无命中，OpenID形状命中已逐项确认为程序标识；diff check及记忆validator通过。此条为发布回执，不宣称五端原生验收通过。后续可在公众平台将1.2.8设为体验版并验收CSV生成/发送及原有流程；不自动切换体验版、提交审核或发布正式版。

- 2026-09-24 发布前核对：用户提出“发布、上传或提交GitHub；发布前需核对准确载荷”。已先完成只读线上代码下载和本地验收，尚未执行发布、上传、提交或推送；已询问此次是仅发布B1，还是同时上传A诊断项、以及分批提交历史已发布代码，等待具体选择。当前main领先origin/main四个既存文档提交，暂存区为空，原有未提交功能和附件保留。

  官方CLI下载当前businessApi成功；新鲜线上6498文件与2026-09-23的1.2.7配套回读基线6498/6498 SHA256相同，无线上漂移。本地76个应用文件仅两个B1仓储不同，依赖/package/config无差异。按两文件白名单准备独立载荷，未把整个脏目录打包。A启动检查点中178个小程序/配置文件只有performance-timing.js变化（8个默认关闭诊断动作）；B1不需要客户端上传，若选择上传会纳入此前已批准A增量，不能称为B1前端改动。

  本轮新鲜本地回归businessApi1423/1423、客户端648/648、容量工具136/136、operationsAnalytics66/66，官方WXML卡片10/前序6/关联8和结构4全部通过；实际发布载荷独立QA后端1423/1423通过，均0失败/取消/跳过。CLI第一次沙箱尝试EPERM且退出0，未误判成功；获准使用官方CLI本地通信后下载明确成功。首次Node准备程序无错误详情退出1，留下载荷和不完整QA支持目录；未发布该中间结果，使用PowerShell补齐本地测试支持后再运行QA。退出原因仍未确认，不改产品代码来规避。

  最终独立载荷检查通过：6498文件，76应用文件与本地一致，仅上述两文件区别于线上；依赖原样保留，QA中的全部载荷文件逐字节一致。清单摘要为9b23b2ca785dc9bbe71ffbd3164be35b62a76e7e02a7ed91dfd844d566ffbcb7。diff check与更新后memory validator通过。本地代码基线目录为临时codex-b1-before-1428371ad9e34b06850ab9c8d1215d69，载荷/QA为codex-b1-release-NfDa07；工具位于outputs/deploy，不能纳入产品或Git提交。线上业务记录、权限、索引、环境变量、Timer、其他函数均未改；原生五端与真实线上CSV运行验收仍unverified。选择发布前需再次核对载荷及线上是否漂移，成功后下载回读验证，不能把预发布验收写成已上线。

- 2026-09-24 用户“实施吧”批准B1执行。按原脏目录文件检查点完成两处运行时改动：完整报告使用每请求独立的来源ID地址表，保留首次/尾部权威事务、重新发现审核票及末尾鉴权；完整基础报告排除未使用的待审核列表，旧仪表/导出默认不变。未改客户端、权限、配置、worker、资源、索引或Timer，未云端读写、发布、上传或Git提交/推送。433个启动文件摘要对照仅两个运行时仓储及一个后端测试变化，零新增/删除，暂存区为空。

  改产品前冻结7组合成完整报告有序行/游标体及9组非导出响应/读取轨迹，改后全部一致；新增97项探针覆盖来源身份/值、审核票增删替换、冷/暖/淘汰/旧游标、撤权、未知读取故障、日期口径及50000行/12MiB边界。TDD已观察省读断言1→0和待审核查询2→0的失败后通过。唯一调整为原离线失败测试预算3000→2000（改后3000可走两页，旧一页断言失败），CLI默认预算不变。无审核节点每页省2次文档读、有审核省3次，再省无关待审核查询；150节点/30售后A样本每页1789→1488读（约16.8%），相同20000读预算11→13页、550→650行，仍test_aborted/complete=false/exit1，不能宣称整份大报告通过。完整smoke33场景30ok/2aborted/1故意缺来源incomplete，零unknown error/写入尝试；月度4500节点与长期5000售后扫描边界未解决。

  新鲜完整回归：工具136/136、businessApi1423/1423、客户端648/648、operationsAnalytics66/66，官方WXML卡片10/10、前序节点6/6、关联分析8/8、结构4/4均通过，0失败/取消/跳过；diff check和memory validator通过。运行命令及范围见 `docs/deployment/export-read-deduplication-acceptance.md`。最终一次独立只读复核通过，无Critical/Important/Minor；复核者独立154/154定向测试并从起始副本复现7+9基线，无修正或再复核。本地自动回归未发现功能回退，但五端原生、真实CloudBase事务/索引/耗时/费用仍unverified，不新增整份报告原子快照承诺，既存功能不属于本批全面审计。B1本地完成，原目录及回退检查点保留。下一步如获准发布先核对精确businessApi载荷；仅因B1无需重传小程序。后续大范围查询/完整导出、首页调度与媒体可靠性需另行设计，不自动提高上限。

- 2026-09-24 用户确认B1书面规格并要求所有现有功能不受影响。完成待审阅实施计划 `docs/superpowers/plans/2026-09-24-export-read-deduplication.md`，4个任务/21个未执行检查步骤：改产品前固化合成报告及旧游标摘要基线；请求内来源ID复用并保留首尾权威核验；仅完整基础报告排除无关待审核列表；内容/权限/竞争/读取量及完整回归、最终统一复核。保持用户已选的连续开发/最后统一复核，不重选执行方式；按writing-plans流程须先审阅本计划，再开始实现。本条覆盖下方“书面规格待审阅”，不意味着产品优化已实施。

  计划限定两个运行时仓储模块，配套测试与离线工具另列清单；页面、客户端、依赖、配置、worker、权限及资源均不改。自查补齐了审核票新增/替换、相同值更换最终反馈、冷/暖/旧游标、原日期口径与完整有序行摘要、报告50000行/12MiB预算，并区分只读测试事务代际与真实CloudBase。保留A的2万读报告中止结果，不能靠调高产品上限宣称容量通过。B1不解决历史扫描/月度4500节点限制。

  本轮仅写计划、更新规格阶段与STATUS；没有实现新探针、测试或产品代码，没有云端操作、发布、上传、Git提交/推送。新鲜验证 `node --test cloudfunctions/businessApi/test/cloud-operations-field-repository.test.js cloudfunctions/businessApi/test/operations-field-analysis-report.test.js cloudfunctions/businessApi/test/cloud-operations-field-analysis.test.js` 31/31及 `node --test tools/capacity/test/*.test.cjs` 39/39通过，均0失败/跳过；仅为现有基线，不是B1收益。启动与结束433个文件摘要（miniprogram/cloudfunctions含既有测试，加project.config）变化/新增/删除均0，暂存区为空。项目记忆validator、git diff --check、计划/规格占位及空白检查通过，计划执行框全部未勾选。下一步用户审阅实施计划后按原执行方式开发；B1完整回归、真实收益、五端及云端验收仍unverified。

- 2026-09-24 用户“按这个推进”批准B1聊天范围：优先减少完整导出重复读取，前端功能、CSV及权限/来源核验保持。完成书面规格 `docs/superpowers/specs/2026-09-24-export-read-deduplication-design.md`，尚待书面审阅；实施计划及产品实现未开始。设计只复用同一导出请求内已验证的固定来源ID，省去尾部事务外的节点/反馈/审核轮次重复预读，仍保留首次与最终事务核验、最终账号/售后检查和新鲜审核票集合扫描。内部引用不进入响应、摘要、游标或缓存，非导出路径不受影响。

  完整报告基础读取拟排除未被使用的待审核列表，原仪表和旧导出保留原默认行为；规格明确披露该变化会消除无关待审核列表故障对完整报告的阻断，等待用户审阅确认，不吞掉相关来源故障。B1不放大容量上限、不改变日期/候选发现，不解决月度4500节点超限；2万读预算的150节点整份报告仍可能未完成，预期减少读取不是实测收益。

  新鲜基线命令 `node --test cloudfunctions/businessApi/test/cloud-operations-field-repository.test.js cloudfunctions/businessApi/test/operations-field-analysis-report.test.js cloudfunctions/businessApi/test/cloud-operations-field-analysis.test.js` 31/31通过，0失败/跳过；只证明现有逻辑。对启动时433个产品/配置文件复算SHA256：变化/新增/删除均0，暂存区为空。文档已自查占位、矛盾、范围和歧义；项目记忆validator、git diff --check及新增文档空白/占位检查通过。本轮仅规格与STATUS改动，保留所有既存脏目录内容，未云端操作、发布、上传、提交或推送。沿用A批次文件检查点而非Git提交；下一步用户批准书面规格后编写实施计划，不将聊天范围批准扩展为跨过后续审阅。B1收益、完整回归、五端和云端验证均unverified。

- 2026-09-24 用户确认容量与可靠性A实施计划；按原目录连续实施、最终统一复核。新增 `tools/capacity/` 离线合成fixture、只读测量适配器、真实仓储场景、独立子进程有界CLI及测试；唯一产品增量为 `miniprogram/utils/performance-timing.js` ACTIONS追加8项，默认关闭/100条内存/异常隔离不变。启动记录433文件SHA256，实施后只此产品文件有变化；页面/云函数实现/配置及所有既存未提交功能保留。本轮无生产连接、发布、上传、资源或业务写入、Git提交/推送。

  修正后新鲜验证工具39/39、businessApi1422/1422、客户端648/648、operationsAnalytics66/66、官方WXML卡片10/前序6/运营分析8、结构4全部通过，0失败/跳过；原安全竞争另单独77/77通过。首轮WXML因未配置本机官方编译器环境变量中止，随后指定既有可信编译器重跑通过。各工具从缺失模块或缺失计时动作的预期失败开始，再验证行为；报告冷续页比较返回数据，不比较随机加密游标字节。只读adapter缺失文档错误对齐原fake字符串，避免把未读标记不存在误认为异常；没有为适配工具修改产品。

  本地39场景31个ok、5个known_limit、2个test_aborted、1个刻意缺来源incomplete；均0写入。150完成节点统计969读，有效派生快照669读；月历史仅查1天150节点2848读；月全4500节点触及现有2000候选节点限制，半年5400/全年10950售后在5000候选售后限制被拒绝。报告v1/v2 150节点首/最后完整页各1789读，2万读预算只完成11页/550行，非完整报告；默认smoke退出1被如实保留。首页100待处理+100待审核+100通知样本1918读/峰值204，94不同归属附件226读/6事务/单事务最多86操作；这些是本地逻辑指标，不是网络耗时、真实并发或线上容量证明。详情和五端/线上/恢复unverified清单见 `docs/deployment/capacity-reliability-acceptance.md`。本批不实施B/C/D；推荐后续优先设计统计候选发现与导出重复复核成本，再处理首页调度。

  一次独立最终只读复核提出4项Important，接受后以失败测试复现，并在一次工具修正批次处理：即使写入拒绝被吞掉仍报error；完整比对统计/分析/v1v2报告内容及重复次数；等量但内容不同的授权数据互换可检出；无明确中止证据的子进程非零退出报error。修正后全套验证及39项基线重跑；无未处理Critical/Important/Minor，不另派第二轮复核。2495组合场景现测172读/158返回/34事务（包含多接口内容核对），最后v2报告9行完整。433个起始产品/配置文件仅计时白名单变化，无新增/删除产品文件，暂存区为空。

  A批次本地完成，不自动发布。项目记忆validator与git diff --check通过；验收表39行与最终机器输出逐行相符，新增工具文件无尾随空白。复核未判断的范围明确保留：既有未提交功能全面代码质量、真实CloudBase类型/事务/索引/计费、五端原生媒体及恢复、B/C实际优化、未部署的真实发布载荷。安卓/鸿蒙/iOS/macOS/Windows原生回归、云端配额费用、媒体增长及隔离恢复仍unverified；任何实际前端行为影响必须先获用户批准。执行决策：保留脏目录并做文件摘要；Bash缺基础命令时改用手工账本；采用文件检查点而非分任务Git提交。下一步设计B统计候选发现和完整导出读路径，仍保留权限/来源复核、全部CSV列及两阶段发送；本条覆盖下方旧的“计划待审阅/未实施”。

- 2026-09-24 用户同意容量优化继续推进，并新增硬约束：目前所有前端功能不得受影响，任何需要保留的实际行为变化必须事先获得批准。已写入PROJECT、规格及首批实施计划 `docs/superpowers/plans/2026-09-24-capacity-reliability-baseline.md`。A计划分6个可验证任务：确定性合成数据、只读适配器、真实仓储场景、有界离线运行器、默认关闭诊断白名单、完整回归及统一复核。唯一计划内产品增量为performance-timing的8个动作白名单；页面、请求工具、云函数实现、上传/审批/联动/CSV协议、权限和线上资源均不在改动范围。

  本轮仅编辑文档及记忆，没有实施计划中的产品/测试工具代码，没有发布、上传、云端读写、提交或推送Git。计划已按规格逐项自查，补齐日期/分页/事务模型、权限竞争、预算中止和诊断异常隔离五类检查，以及安卓/鸿蒙/iOS/macOS/Windows独立验收；自动回归不能替代原生验收。新鲜只读验证 `node --test miniprogram/test/performance-diagnostics.test.js miniprogram/test/cloud-performance.test.js miniprogram/test/diagnostic-isolation.test.js` 为10/10、0失败/跳过，仅证明现有诊断基线，不证明新工具完成。文档占位扫描未发现未完成占位，git diff --check与项目记忆validator通过。下一步用户审阅实施计划后，建议保留连续开发、最后统一复核的方式；新工具、容量收益、五端和云端性能仍unverified。下面更早条目中的“书面规格待审阅/实施计划未开始”已由本条推进状态覆盖。

- 2026-09-24 用户同意推进现有架构容量与可靠性优化，并将容量输入修正为约50名员工、30条售后/天、每条图片视频合计平均100MB（取代50MB估算）；这不是修改单轮120MiB上传限制或承诺50并发。已核对当前统计查询边界、导出来源重验、首页只读性能样本及现有内存诊断。形成待审阅规格 `docs/superpowers/specs/2026-09-24-capacity-reliability-baseline-design.md`：保留云开发，A先落地可重复基线/可选诊断/验收清单，B统计导出、C首页、D真实隔离容量与恢复按证据分批设计。不把A完成误称为容量问题已解决。

  本轮仅更新设计文档及规划输入，未修改产品/测试代码、部署、上传、索引、Timer、套餐或业务数据。新鲜只读基线 `node --test cloudfunctions/businessApi/test/dashboard-performance.test.js cloudfunctions/businessApi/test/business-read-performance.test.js cloudfunctions/businessApi/test/cloud-operations-field-repository.test.js` 为42/42、0失败/跳过；这验证现有逻辑和边界，不证明容量目标已达成。规格已自查范围、测量口径、100MB参数和无真实数据边界；文档差异检查与项目记忆validator通过。仅新规格独立本地提交为 `6bee3e7`，未推送GitHub，既存代码和项目记忆改动未混入提交。书面规格尚待用户审阅，实施计划和开发未开始；真实云端配额、成本、并发、清理积压、备份恢复及五端性能仍unverified。下一步用户确认书面规格后编写A的实施计划。

- 2026-09-23 18:19–18:21（上海）按已确认授权完成运营关联分析发布：官方CLI `cloud functions inc-deploy` 明确成功，businessApi载荷6498文件/9.0MB；随后再次官方下载并用 `outputs/deploy/verify-unified-card-release.cjs` 全量比较，6498/6498文件SHA256一致、差异0。`verify-operations-analysis-stage.cjs` 确认当前应用76文件完全对应本地，其他线上依赖及包文件未变；载荷清单摘要cf40c0988eea48545119c78b292c7e6b63d107d434d11feb13e5e3f0b97fa60d。仅8应用文件有本次变更（4新增），其他已发布功能保留；没有发布operationsAnalytics或改变资源、权限、Timer、环境变量、模板、业务记录。

  小程序1.2.7官方CLI明确 `√ upload`、退出0，AppID核对正确，包1397141字节，上传信息 `outputs/deploy/miniprogram-1.2.7-20260923-field-analysis-info.json`。包含本次看板与此前批准的前序节点交互优化。project.config.json SHA256仍为D15B47C22F1F3A1E04236715D889B05B7C9AC935B0AC721C39B7E3B8A514D2DC；未切换体验版、未提交正式审核或正式发布、未提交/推送GitHub。

  最终本地及载荷后端1422/1422，客户端648/648，统计worker66/66，官方分析WXML8/8、前序6/6、卡片10/10、结构4/4，bundle检查、四种宽度合成布局、git diff --check及项目记忆校验通过。唯一独立复核的5个问题已在一轮修复中先红后绿解决，无延期Minor；无二次复审。没有可供本轮授权使用的真实业务登录会话，因此新接口线上业务查询、五端原生体验和生产性能仍unverified；代码回下载验证不等于这些验收。下一步用户在公众平台把1.2.7设为体验版，按 `docs/deployment/operations-field-analysis-acceptance.md` 检查节点折叠、商品逐层/属性组合、双字段、CSV和前序节点返回，不擅自创建生产测试售后。

- 2026-09-23 运营关联分析统一独立审查及修复完成：审查无Critical、3项Important，另2项名义Minor按实际误导分析的影响升级修复。分别复现并先红后绿修正：展示代际变化不能屏蔽同会话撤权；来源变更必须失效待发送CSV；v2报告强制上下文与新元数据，旧协议验证只在旧路径兼容；关联版本仅列实际兼容维度的记录；组合按“字段名：原值”逐项显示避免分隔符歧义。没有二次派发复审，所有问题以失败测试和完整回归验证。独立审查自身另跑后端186/186、客户端80/80及10组旧协议黄金比较，不代替主代理最终全量。

  修复后最新完整回归：businessApi1422/1422、客户端648/648、operationsAnalytics66/66，均0失败/跳过。保留线上依赖的实际发布载荷在隔离QA再跑1422/1422；6498文件/76应用文件逐个对比正确，排序路径与SHA256清单摘要cf40c0988eea48545119c78b292c7e6b63d107d434d11feb13e5e3f0b97fa60d。用户已确认平台最新1.2.6，本次上传1.2.7。代码/包验证完成，下一步执行已授权发布和上传；本条不代表发布已完成。五端真机、真实云端延迟/事务表现和用户会话中的新分析查询仍unverified。无延期Minor；既有功能完整性由既有测试及本轮回归覆盖，不另扩大重构；不物理清理旧CSV文件、不扩展全局认证治理。

- 2026-09-23 用户确认执行后，在已批准的现有工作区连续实现运营字段关联分析（ADR-0022），原有未提交改动保留。后端新增历史来源关系投影、只读分析和版本2导出；客户端独立组件实现零记录隐藏、节点折叠/前10项、商品逐层与实际属性组合、同节点双字段分析。旧快照/摘要、工时口径、旧接口及两阶段 CSV 保持；当前未更改权限、集合、Timer、模板或业务数据。

  本地新鲜验证：businessApi 1421/1421、operationsAnalytics 66/66、客户端645/645，官方WXML新分析6/6、前序节点6/6、原卡片10/10、结构4/4，均0失败/跳过；四文件bundle同步检查通过。额外边界先红后绿验证重复节点不重复计数、首次查询有限代际、折叠清理控件/加载状态、无效双字段清旧上下文、导出条件一致、明细与严格统计组对应。320/375/430/768px合成Chromium布局及375px图像检查通过，不代表五端原生验证。

  正在进行唯一独立整体复核及发布前基线检查，尚未发布或上传。官方IDE登录true；下载生产businessApi共6494文件，仅8应用文件与本次目标有差异，其中4新文件；依赖及已有上传/卡片/前序节点代码一致。project.config.json SHA256仍为D15B47C22F1F3A1E04236715D889B05B7C9AC935B0AC721C39B7E3B8A514D2DC。公众平台浏览器访问被安全策略拒绝，未绕过；最新上传本机记录1.2.6，已请用户确认是否存在更高版本。五端真机及线上新分析查询均unverified，复核和载荷验证完成前不发布。验收清单见 docs/deployment/operations-field-analysis-acceptance.md。

- 2026-09-23 用户已明确确认运营看板关联分析书面规格。本轮形成 `docs/superpowers/plans/2026-09-23-operations-field-analysis.md` 七阶段实施计划，等待计划审阅；执行方式保留为当前目录连续开发、最后统一独立复核，测试通过后发布 businessApi 和上传新版小程序的授权保持。自查明确旧查询叶子模块提取以避免循环依赖，新增关系描述只作为已核实来源的查询投影，旧快照生成和 sourceDigest 不变；同步 worker 的新增纯 helper 不等于要求发布 operationsAnalytics。当前仅编辑规格确认状态、计划和本条记忆，未修改产品/测试代码、部署函数、上传或推送 Git。

  本轮只读基线 `node --test cloudfunctions/businessApi/test/operations-field-domain.test.js cloudfunctions/businessApi/test/operations-field-service.test.js cloudfunctions/businessApi/test/cloud-operations-field-repository.test.js cloudfunctions/businessApi/test/operations-report-cursor.test.js miniprogram/test/operations-field-flow.test.js` 实际113/113、0失败/跳过。读取本机官方 CLI 的 download/inc-deploy/upload --help，未执行云端读取或写入；参数写入计划但运行时需重核登录、目录和最新版本。基线不证明新分析已实现或通过；新功能、发布、五端验收仍 unverified。下一步用户确认实施计划后按七阶段执行，不重复确认发布授权。

  计划自查已补齐关系描述至响应/客户端的 linkages 与 productStage 契约，并核对五类重点风险各有归属测试。`git diff --check`、未完成标记扫描及项目记忆 validator 通过；文档仍留在工作区，没有提交本轮计划或夹带既存改动。

- 2026-09-23 用户批准运营看板“零记录隐藏、节点折叠、商品逐层分析、同节点双字段分析、关联 CSV”范围，并授权测试和统一复核后发布 businessApi、上传新版小程序。当前仅完成书面规格 `docs/superpowers/specs/2026-09-23-operations-field-analysis-design.md`，等待规格审阅；未实现、测试新功能或执行发布。只读核对确认零次候选由 aggregateUnique 初始化后全部展示，现有快照保留同节点实际选择值但不携带关系元数据。设计采用独立查询分析模块、历史实例关系验证和显式新版报告，保留共享快照及 operationsAnalytics 协议、旧客户端兼容和原工时/基础导出口径；不改集合、Timer、权限、模板或业务数据。既有未提交上传、卡片、前序节点改动全部保留。下一步规格确认后编制实施计划，再按先测后改、最终统一复核推进；功能、发布和五端验收均 unverified。

  文档自查完成，`git diff --check`、`git diff --cached --check` 和 `python C:/Users/87579/.codex/skills/maintaining-project-memory/scripts/validate_memory.py .` 均通过。仅规格文件本地提交为 `08e2e44`，未推送；首次暂存被只读 .git 权限拒绝，正常获准后以显式单路径完成，未夹带既有代码或记忆改动。本轮没有运行产品测试，不能沿用上一功能测试结果宣称新分析已通过。

- 2026-09-23 用户批准按架构评估方案优化前序节点记录，并明确在当前未提交工作区原地修改、保留已有已发布代码。本轮仅修改共享 previous-node-records 的 JS/WXML/WXSS 及处理/审核页的返回事件与锚点；未触及上传、审批写流程、业务数据或后端实现。历史处理和各轮审核独立加载、错误/重试分离，每条保存与每轮审核默认折叠，下一审核页失败保留已加载页与游标。按钮全宽、字段紧凑键值排版、附件名省略与操作状态分栏；视频改为固定弹层，提供打开中/取消/关闭/原生播放失败提示。“返回当前填写/审核”先收起再滚动，真实页面草稿、上传队列及未提交意见不变。

  保持安全边界：当前已授权节点内单条折叠不再查询；整区收起、切换节点、页面隐藏、账号/权限失效仍清空，迟到结果不能回填。没有引入跨账号或跨节点缓存，也没有删减服务端权限/最终来源复核。修改前基线针对性27/27通过；新增交互测试先出现预期失败（新方法缺失、仍耦合历史、隐藏条目仍可请求附件等）。独立只读复核发现1项Important：新取消入口会忽略迟到的同上下文权限拒绝。以FORBIDDEN/ACCOUNT_DISABLED两项用例先红后绿修复，并补充旧节点拒绝不能清空新节点的保护；取消仅失效展示，权限拒绝仍清空该查看上下文。复核无Critical/其他Minor，复核者另外运行17项边界探针。该项修复由回归验证，未重复派发复核。

  最终新鲜组件/页面行为19/19、完整客户端627/627、官方WXML6/6通过；同轮businessApi1390/1390、原卡片官方WXML10/10、WXML结构4/4通过，均0失败/跳过；WXSS官方编译退出0并输出2988字节。git diff --check与项目记忆验证通过。原生视频/触摸、窄矮窗口、线上版本竞争及性能未作为复核验收结论，继续留作五端实际验证。

  使用本机临时合成数据及真实WXML输出转换进行Chromium布局检查，320/375/430/768px四种宽度均验证按钮全宽、无水平溢出、附件操作保留宽度，并查看375px渲染图；相同断言对修改前基线在320px失败（标题按钮159px/节点按钮107px，内容宽273px）。这是模拟布局，不等于微信原生或五端验收。截图与探针只在系统临时目录previous-node-polish-baseline-d516348386ea48d69830bab2f0432614，不含真实业务记录、不进入Git。

  首次读取性能按先测量后决策处理：真实只读仓储+fake DB合成0/6/40/94附件得到20/32/66/132次串行I/O、3/4/4/6事务，单事务最多7/12/46/46操作，0写入；不能据此把录屏约7秒归因于某个云端阶段。没有实测云函数冷启动/网络/数据库耗时，因此本轮不改后端并行或授权结构；首次延迟仍unverified。getPreviousNodeResult仓储SHA256与本轮前相同，project.config.json SHA256仍为D15B47C22F1F3A1E04236715D889B05B7C9AC935B0AC721C39B7E3B8A514D2DC。未发布函数、上传小程序、提交/推送Git、调用线上业务或改权限/定时器。本地交互优化及自动回归已完成；下一步经授权上传新版客户端，再按docs/deployment/previous-node-records-acceptance.md作五端预览/返回/草稿验收；本增量不需重发businessApi。首次读取慢另按真实请求分阶段计时定位。

- 2026-09-23 按用户“发布businessApi并上传”授权，完成前序节点记录功能的businessApi代码发布及小程序1.2.6上传，覆盖下方本地实现“未部署/未上传”的状态。先通过官方微信开发者工具CLI下载生产基线，72份本地应用/包文件中仅index.js、lib/cloud-feedback-repository.js、lib/node-workspace-service.js有修改，并新增lib/previous-node-result-repository.js；其余应用文件、package.json、锁文件与生产一致。保留下载的生产node_modules，以inc-deploy目录方式一次发布6494文件。再次官方CLI下载，6494/6494文件SHA256逐字节一致，文件列表与内容均无差异。

  发布前新鲜回归：businessApi1390/1390、客户端615/615、官方WXML前序组件及页面4/4、既有卡片渲染10/10、WXML结构4/4，均0失败/跳过。实际发布载荷复制到隔离QA目录并补齐测试所需关联源码/文档，再次完整后端1390/1390通过。CLI上传1.2.6明确返回“√ upload”、退出0，包大小1332785字节；信息文件outputs/deploy/miniprogram-1.2.6-20260923-previous-nodes-info.json。发布脚本outputs/deploy/stage-businessApi-20260923-previous-nodes.ps1复用既有线上依赖保留方式，回读复用verify-unified-card-release.cjs。

  本机临时生产基线目录codex-previous-node-before-1ca2d9e19f3d4133a7e7db590272296c，发布载荷/隔离QA目录codex-previous-node-release-6bb558fb835448068445da2a0e7d8994，回读目录codex-previous-node-after-a52c930285064f1dbd1a2650b8d5831a；只用于代码验证，不包含业务数据备份。首次受沙箱限制的CLI尝试报本机通信文件EPERM，未将其退出0当作成功；正常提权后下载、部署和上传均成功。浏览器控制台登录已过期，未要求用户扫码，未执行线上业务调用或运行时启动测试；实际功能、线上性能、公众平台体验版指向及五端真机仍unverified。

  project.config.json发布前后SHA256一致。没有写业务记录、模板、账号、索引、权限、环境变量、Timer或函数资源配置，没有变更依赖、发布其他函数/企微工作区、提交正式审核/正式发布或提交推送GitHub。发布后项目记忆验证与git diff --check通过。下一步确认公众平台体验版指向1.2.6，再依docs/deployment/previous-node-records-acceptance.md在处理和审核页验证前序节点最终内容/附件/审核意见、历史分页及当前草稿不受影响，原有上传/提交/审核作五端回归。

- 2026-09-23 按用户批准的方案本地实现“前序节点记录”：处理页与审核详情页共用只读折叠组件，按实际流转路径列出前序已完成节点，跳过未走分支；未展开不新增查询，展开节点默认读取最终通过快照或无需审核节点的完成反馈，历史保存及各轮审核独立加载。展示字段、处理说明、处理人快照/提交和完成时间、该最终处理轮的全部凭证及审批意见；不把最后一次保存误作最终结果，不扩大审核操作权限，不修改当前处理草稿或审核意见。

  新受保护读接口 `getPreviousNodeResult` 复用既有最终来源证明，授权使用当前售后字段权限而非全局卡片可见性。凭证与投票按40条分块固定文档核验、所属反馈去重，每块及最终响应前重新验证权限与不可变来源，单事务不超过100次操作，不引入文件数量上限。`getNodeHistory` 补充查询后的账号/成员/来源复核，以及按实际提交人对应实例参与人姓名快照的安全标签；不查询当前账号姓名补写历史。旧流程不请求其不支持的审核历史接口，仍保留处理历史回退。沿用原凭证访问授权、过期清理和多端基础组件，不改业务数据、集合、索引、Timer或依赖。

  新增回归先红后绿；独立首轮只读复核发现历史查询返回前授权缺口、单事务附件读取预算、旧节点审核接口兼容三项，以及可恢复处理人姓名未展示的问题，均已形成永久失败用例并修复。额外测试覆盖快速切换节点、账号/角色失效、页面隐藏/卸载、收起后迟到视频授权、审核历史分页、损坏意见拒绝、94个不同反馈所属附件及1001个附件读取且所有事务<=100次操作。完整后端首轮因新增末尾权限复核令性能测试31轮变为34轮而失败；核对仅增加账号/售后/节点3次固定读取后更新精确断言，并保留四路并发、101条历史完整性、只读及无事务查询断言。

  最终新鲜 `npm.cmd test --prefix cloudfunctions/businessApi` 1390/1390、`node --test miniprogram/test/*.test.js` 615/615、官方编译器 `node tools/test-previous-node-records-rendering.mjs` 4/4、既有卡片渲染10/10、`node tools/test-wxml-structure.mjs` 4/4通过，均0失败/跳过；项目记忆验证与 `git -c core.safecrlf=false diff --check` 通过。第二轮独立只读复核通过，无遗留Critical/Important/Minor；复核者另行验证135项相关测试、47项性能/功能测试及23项合成边界探针。复核明确不覆盖线上性能、五端原生行为及既存上传/卡片改动，这些未作为本轮已验收结论。`project.config.json` SHA256与本轮前相同，既存未提交上传/卡片修复保持；未提交/推送Git、未部署云函数、未上传或发布小程序。五端真机、线上性能及本次功能线上验收仍unverified。下一步按发布授权部署businessApi并上传新版客户端，再依 `docs/deployment/previous-node-records-acceptance.md` 验收。

- 2026-09-22 按用户“发布并上传新版小程序”授权，完成售后卡片统一的businessApi发布和客户端1.2.5上传，覆盖下方本地实现“未部署/未上传”的状态。本轮先用官方开发者工具CLI下载生产基线，71份应用/包文件仅index.js与lib/business-card-service.js两份不同；保持生产node_modules、package.json及锁文件，未夹带本地依赖漂移。发布载荷共6493文件，以官方inc-deploy目录方式一次更新，控制台刷新显示上次部署09-22 15:24:23、状态正常。再次官方CLI下载后6493/6493文件SHA256逐字节一致，无任何差异。在线编辑器仍有既存未部署草稿，本轮未点击其“部署”或“回退修改”。

  发布前新鲜验证：客户端608/608、businessApi1369/1369、官方WXML卡片渲染10/10、WXML结构4/4通过。相同发布载荷复制到隔离QA目录，首次因遗漏测试所读取的部署文档而失败；补齐文档及关联工作器源码（仅QA目录，不进入载荷）后完整1369/1369通过，0失败/跳过。生产默认无action事件调用成功，返回预期UNKNOWN_ACTION、耗时11ms；只验证模块启动和入口拒绝，不写业务数据，不代表真实卡片字段验收。

  官方CLI上传小程序1.2.5返回“√ upload”、退出0，包大小1305871字节；结果文件outputs/deploy/miniprogram-1.2.5-20260922-unified-cards-info.json。生产前/后代码分别保存于本机临时目录codex-card-release-before-6712d60168db4e9ab687477970bcddcb、codex-card-release-after-45e6fa7da09c4b0eb24c803301c04409，验收载荷在codex-card-release-aced7303ea1240938d01fe0727660056/payload/businessApi；这些只用于代码核对，不保存业务记录或密钥。打包与回读校验脚本在outputs/deploy/stage-businessApi-20260922-unified-cards.ps1和verify-unified-card-release.cjs。

  project.config.json发布前后SHA256一致；未修改业务记录、模板、权限、环境变量、Timer、依赖或函数资源配置，未部署其他函数/企微工作区，未提交正式审核/正式发布或提交推送GitHub。项目记忆验证和git diff --check通过。公众平台体验版指向1.2.5及iOS/Android/HarmonyOS/Mac/Windows真机仍unverified；下一步确认体验版版本，对同一售后比较首页、全部售后/检索、待处理、待审核的编号和模板字段，保留各自节点/轮次/截止/操作入口，并核对摘要重试与账号切换。

- 2026-09-22 用户确认售后卡片统一范围，本地将“待我处理”“待我审核”接入首页/全部售后/检索结果已使用的共享展示工具、WXML及WXSS。标题以售后编号为先，不再同时展示生成名称和重复编号；字段按模板配置而非硬编码四个名称，空配置与不可用/重试沿用首页语义。待办仍保留节点编号、轮次、截止、分支/追加决定、处理及审核导航，会签/或签保留；仅审核卡片把轮次pending展示为“待审核”，没有改变权威状态。

  businessApi仅为两个已有待办读入口添加共享摘要装配，明确使用businessLineId而非节点/轮次_id，沿用已有请求内去重、4路并发和每次字段授权；不增加逐卡详情请求、集合、Timer或业务操作。客户端新增字段仅在内存，账号/角色变化及受保护接口失效时清空卡片和分页，旧请求不得覆盖新会话；网络失败保留同账号可见内容供重试。

  先红后绿证据：后端新增3项、客户端26项、官方WXML新增4项均在修改前观察到预期失败。首次完整后端回归失败于business-card-integration旧断言“待办不返回摘要”，核对真实默认运行链路后改为断言准确摘要及原节点身份/状态，不删覆盖。最终完整客户端608/608、businessApi1369/1369、官方WXML卡片渲染10/10、结构4/4通过，均0失败/跳过；独立只读代码复核无Critical/Important/Minor，复核者另行运行目标79/79、相关95/95及内存竞态探针10/10，确认迟到分页、页面卸载及分支/追加决定导航。复核不覆盖既存上传改动、线上性能或原生设备，这些不纳入本次完成声明。项目记忆验证和git diff --check通过。project.config.json SHA256与本轮前一致，既存上传修复保持。未部署云函数、上传客户端、提交/推送Git，未更改线上业务记录、权限或定时器；五端真机和线上展示仍unverified。下一步按单独授权发布businessApi、上传新版小程序，由原设备对比同一售后的各入口卡片。

- 2026-09-22 已按用户“发布并上传”授权完成批量凭证恢复上线，覆盖下方“未部署/未上传”的本地交付状态。CloudBase控制台将businessApi部署到当前环境$LATEST，显示部署时间09-22 14:25:14、状态正常；仅替换index.js和lib/cloud-evidence-upload-repository.js。部署前发现本机node_modules与线上依赖有差异，因此保留线上依赖、包清单和锁定版本，未夹带本机依赖升级。部署包为本机outputs/deploy/businessApi-20260922-bounded-evidence-recovery.zip，6493个文件、9473340字节，SHA256为C4A8C7E0563812832A7F0EA014C60632A96B5CA705A2AA70867C4C37A1BD1D21。

  发布前新鲜客户端582/582、businessApi1366/1366、WXML结构4/4通过；另外将实际ZIP解压到隔离临时目录，补齐测试所需相对路径后再次完整回归1366/1366通过。首次临时测试目录布局不符，已中断并重新建立正确布局，不计为通过。官方CLI回下载6493个线上文件，6492个与部署ZIP逐字节SHA256一致，唯一差异node_modules/.package-lock.json经JSON深比较确认仅license元数据变化，依赖版本、完整性和实际依赖代码不变。云端默认无action测试显示调用成功，返回预期UNKNOWN_ACTION、耗时12ms；仅验证启动和入口拒绝，不写业务记录，不代替真实上传验收。

  官方微信开发者工具CLI上传小程序1.2.4，返回“√ upload”、退出0，包大小1295699字节，信息文件outputs/deploy/miniprogram-1.2.4-20260922-batch-evidence-upload-info.json。首次沙箱登录检查EPERM，按正常提权获准后确认login=true；不将首次退出0视为成功。浏览器文件选择被用户误关，用户要求重开后重新选包完成部署；未误发布在线编辑器残留草稿。原project.config.json发布前后SHA256一致，未改业务数据、模板、权限、环境变量、Timer、内存或超时，未部署其他函数或企微工作区，未提交正式审核/正式发布，未提交或推送GitHub。

  项目记忆验证和git diff --check通过。公众平台体验版是否指向1.2.4、原设备多视频实测与五端真机仍unverified；下一步确认体验版版本后，用原失败批次复测多选→并发传输→逐个登记→暂态失败最多3次应用层尝试→保存及重新进入。已成功文件不应重复上传；永久拒绝不自动重试；25秒提交耗时仍为独立待查问题。

- 2026-09-22 用户同意批量凭证最小恢复方案，要求不影响其他功能、避免堆叠代码；本轮仅本地实现和复核，未授权自动发布。保持每页最多3个并发传输，同一上传操作的核验登记串行；传输和登记各最多3次应用层尝试（初始+2次），登记失败只重放原预约ID/令牌/版本，不重传字节或另建预约。失败不阻塞后续文件；成功项保留；页面显示阶段/次数，耗尽后提示重试该文件。准备文件、首次预约、永久业务拒绝及未知内部异常不盲重试；手动重试沿用原失败文件流程，不宣称跨页面持久续传。

  本地修改集中于已有上传工具、节点页状态显示、上传仓储和API安全错误出口；没有新增通用重试框架、后台任务或数据库结构。仓储对读取事务明确冲突及暂态存储错误返回固定EVIDENCE_UPLOAD_RETRYABLE，日志只记录固定阶段/原因分类；沿用既有提交事务冲突重试、120 MiB、角色、版本、签名和幂等约束。重试/排队/续期前后检查当前页面账号状态。25秒保存提交问题、审批、模板、统计、提醒、权限及Timer没有改动；project.config.json与既有1.2.3选择器修复保持。

  只读线上核对已完成，覆盖下方“尚未读取线上日志”的旧状态：目标时段两个finalize请求返回INTERNAL_ERROR，后续登记成功；日志无底层原因，不能认定生产根因为事务冲突。部署代码与本地基线核对一致。最终保存提交成功约25.4秒，同期两次派生检索刷新合计约8.7秒，不能据此解释全部耗时；本轮不调整派生链路。没有把原始日志、业务标识或凭据写入仓库。

  新增测试先红后绿，覆盖读取冲突分类、队列隔离、3次预算、权限/版本停止、SDK嵌套错误、失效守卫、尝试提示及真实上传器连接合成仓储的响应丢失回放；后者确认1次传输/1条凭证/容量只计1次。首轮独立只读复核发现403纯文本凭据过期被提前脱敏、SlowDown/503及InternalError/500被新增白名单挡住两项回归；分别新增失败测试后修正，恢复原始错误内部分类但仍只输出脱敏结果，永久错误及API的INTERNAL_ERROR不重试。复核者再次以锁定COS SDK仅替换网络边界确认恢复，所有场景1次预约、原身份/对象键不变，无遗留Critical/Important/Minor。

  最终新鲜 `node --test miniprogram/test/*.test.js` 582/582、`npm.cmd test --prefix cloudfunctions/businessApi` 1366/1366、`node tools/test-wxml-structure.mjs` 4/4通过（0失败/跳过），JS语法、`git diff --check`、项目记忆验证通过；两次全量均实际检查测试汇总，未把被管道掩盖的退出状态当作证据。未部署businessApi、上传小程序、提交或推送GitHub，五端真机与原视频仍unverified。下一步另行取得发布businessApi及上传新版客户端授权，再用原设备验收；不能将本地通过表述为线上视频问题已解决。

- 2026-09-22 用户确认体验版1.2.3仍无法一轮完成多视频上传。本轮只读查看用户录屏：6个视频已选入，合计约8.6MB；首轮部分在“文件核验”阶段报EVIDENCE_UPLOAD_FAILED，再次保存后逐步全部已登记，之后仍约二十余秒处于提交中。录屏不包含服务器原始异常，也未确认最后保存结果；不能归因于多选限制、文件格式、容量或昨日选择器修复，也不能从外观确认实际系统型号。

  当前源码调查：每页最多3个文件并发，finalize提交事务有明确冲突的有限重试，但preflightFinalize读取事务直接调用runTransaction；客户端对登记失败重新开始预约/传输，无普通登记阶段恢复。锁定wx-server-sdk仅替换数据库网络边界，复现document.get将DATABASE_TRANSACTION_CONFLICT包装为errCode=-501001并只执行一次读取；再以合成数据注入该已验证错误形状，当前finalizeUpload一次即失败，delay调用0次、存储访问0次。这证明读取阶段存在未保护路径，但本次线上根因仍unverified，不能把注入测试视为生产取证。既有仓储18/18回归通过。原生SDK探针首次错用queryDocument动作、合成探针首次受VM跨域Date影响，两者纠正后才取得上述证据，未修改产品文件或测试。

  已打开目标businessApi日志页，CloudBase登录过期，已请用户登录原账号以核对2026-09-22约12:26请求及耗时。尚未读取线上日志、未部署/上传/提交Git、未修改任何业务数据；既存工作区改动保持。录屏抽帧仅在系统临时目录分析，不进入Git或项目记忆。下一步登录后只读确认部署/请求日志，区分核验读取冲突、提交冲突或网络超时，再确定最小修复和验收；全部登记后等待时间另行核对submitFeedback及派生步骤，不能无证据归因。

- 2026-09-21 已按用户“上传”授权，通过官方微信开发者工具 CLI 从当前原工作区上传小程序 **1.2.3**，返回 `√ upload`、进程退出0，包大小 **1272990字节**；上传信息文件为本机 `outputs/deploy/miniprogram-1.2.3-20260921-video-picker-upload-info.json`。版本说明为“修复视频选择失败：原视频选择、本机视频兼容重试、脱敏诊断和草稿保护”，包含新文件 `miniprogram/utils/evidence-picker-error.js`。上传前新鲜客户端回归570/570、WXML结构4/4、`git diff --check`通过；官方CLI确认已登录。首次沙箱调用因访问开发者工具本地配置被EPERM拒绝，获准正常提权后登录检查及上传成功，不把首次进程退出0视为成功。

  本次只上传客户端，未部署businessApi、提交正式审核、正式发布、提交或推送GitHub；未修改业务记录、权限、Timer或既存project.config.json，也未混入企微隔离工作区。当前上传成功覆盖下条“尚未上传”的状态；公众平台体验版指向、原Android视频上传及五端实机结果仍unverified，上传不能等同真机修复验收。下一步由用户确认1.2.3为体验版，退出后重新进入，并使用原失败视频验证选择→上传登记→重新进入；如仍失败，提供固定脱敏诊断码，不收集原始路径/内容。

- 2026-09-21 视频选择修复本地交付结论：独立复查已无Critical/Important问题，剩余引号包围的绝对路径误分类细节已补2项失败测试后修正。最终完整客户端570/570（目标页86项）、businessApi1362/1362、WXML结构4/4、官方WXML编译6种渲染状态通过，项目记忆验证器通过；所有真实Android故障根因、原视频上传和五端实机结果仍unverified。本条覆盖下方“复核进行中”的中间状态。修改仅在本机，未上传、部署、提交或推送；需要经授权上传新版小程序后由原设备复测，本次无须改发businessApi。若仍有失败，凭固定诊断码继续调查，不将本地修复表述为真机问题已解决。

- 2026-09-21 视频选择修复追加复核：首轮只读复核指出批量中单个无效响应连带拒绝有效文件、版本变化残留无效重试入口、路径词语影响错误分类三项问题。新增6项测试先全部失败，逐项修复后完整客户端568/568、目标页84/84（包含在完整集）、WXML结构4/4、JS语法和diff检查通过；下方562/78为修复前中间结果。现按文件过滤无效条目，保留有效条目并给出部分失败提示；工作区更新版本/只读状态或过期重试时清除入口；错误分类先移除URI及绝对路径尾部。复核者正在进行针对性复查。Android原视频和五端真机仍unverified，未上传/部署/推送。

- 2026-09-21 节点视频选择失败的客户端修复已完成本地实现，尚未上传或真机验收。用户确认疑似Android、选好视频后出现统一选择失败提示；尚无原生错误码或故障视频，真实触发原因仍为unverified，不能认定为压缩失败或后端故障。已按批准范围增加固定分类及有界数字错误码的页内诊断、结果结构验证、异步处理异常捕获、重复点击/回调防重；支持原视频参数的移动端显式original；仅chooseMedia原生媒体处理/读取/未知失败允许用户手动chooseVideo相册/不压缩重选。取消、权限和隐私拒绝不重试，不转聊天，不记录原始错误/文件路径/内容；重试沿用账号/节点/版本/锁、120MiB和白名单校验。Mac/Windows原路线及PDF来源保留，businessApi和业务记录未改。

  基线客户端536/536、目标页52/52通过。TDD新增诊断/重试用例先出现预期失败；第二轮确认7项失败（3端原视频参数、缺失/空响应、无路径文件），修复后目标页78/78、完整客户端562/562、businessApi1362/1362、WXML结构4/4均退出0。官方wcc对节点反馈页编译及6种错误/重试/权限锁渲染状态通过（非真机），JavaScript语法及git diff --check通过。npm仅显示既有用户prefix/cache配置警告，Git仅提示LF/CRLF转换；未修改这些配置。独立只读代码复核尚在进行，后续结论另补。本轮直接在原工作区修改范围内干净文件，保留原project.config.json、附件、部署输出及缓存；不混入企微隔离工作区，不提交或推送GitHub，不部署云函数、不改权限/Timer/费用。下一步完成复核，再经授权上传新版，用原失败视频在目标Android微信验证选择→上传登记→重进，并按五端矩阵记录；如果仍失败，只收集脱敏诊断码继续定位，不能宣称五端实机已通过。

- 2026-09-21 用户批准按已审计划连续本地开发、末尾统一独立复核，覆盖下方设计/计划待审状态。开始准备隔离工作区和基线验证；不包含生产部署、真实发送、购买、扩大权限或Timer。实施结果以随后测试和提交证据为准，不能将批准视为完成。

- 2026-09-21 用户已批准企微绑定书面设计并要求进入实施计划。本轮新增 `docs/superpowers/plans/2026-09-21-wecom-binding-and-verification.md`，12个任务覆盖本地绑定、本人确认、签名发送/去重、容器适配、双端页面、回归及另行授权后的单人上线验收；计划尚待用户审阅和选择执行方式，未开始产品代码。设计文件状态已更新为批准；此条取代下条“设计待审”的当前状态。

  只读核对源码发现正常登录也递增credentialVersion，因此计划把短期挑战的凭据版本约束与长期授权的独立wecomSecurityEpoch分开；密码重置、停用及身份解绑仍失效，普通登录不自动撤销已确认授权。新增文档还明确错误验证码计次必须事务提交、成员滚动限流跨改绑保留、以challengeId持久去重、发送后崩溃不得自动重发。官方企微接口正文仍无法读取，CloudBase固定出口页本次抓取超时；官方SDK初始化文档已读取，但普通容器实际鉴权/传统数据库事务形状、UserID规范及消息契约仍unverified，任务7设置硬门禁。未查看或操作云控制台、通讯录或真实密钥，未部署、上传、发消息、改Timer或推送GitHub。本轮只有文档改动，不重跑产品测试；git diff --check、计划结构检查（12任务/67检查项、代码围栏成对、无未完成占位标记及尾随空白、高置信度凭据模式0命中）、项目记忆验证器均通过。人工自查修正了方法计数、共享错误类型依赖、版本递增与占位同步、崩溃后验证码可确认、成员契约默认关闭等计划歧义。记忆校验曾因本段引用占位标记的字面拼写而失败，已改为描述性文字后重新验证。原project.config.json、部署及说明输出、Python缓存继续保留且排除本次范围。下一步用户确认计划及执行方式，再隔离工作区按任务实施；生产配置/费用/测试接收人单独明确。

- 2026-09-21 企微提醒接入进入设计阶段，用户选择“管理员绑定企微成员 UserID、员工本人确认收件”。已形成待审设计稿 `docs/superpowers/specs/2026-09-21-wecom-binding-and-verification-design.md`，第一期限定人员绑定与不含业务内容的验证码验证发送，正式小时提醒另阶段接入；尚未获得书面设计批准，不得直接实施。当前源码仅预留 `wecomUserId`，提醒工作器只写站内通知；本日此前 `npm.cmd test --prefix cloudfunctions/workflowReminder` 39/39通过，不代表企微API或双端收件已验收。

  同日只读控制台证据：目标环境仍为个人版，`workflowReminder` 公网访问开启、固定出口未启用且页面标为标准版能力，Timer为空；云托管固定公网出口已开启，但服务清单0条。云托管与云函数出口不得混用，实际出站与企微可信配置仍unverified。镜像仓库曾提示云角色授权，未授权；所有配置编辑均取消，未开关网络、创建服务、发送通知、读取通讯录、部署或购买。只保存脱敏条件，不记录具体IP、成员标识、密钥或原始页面内容。用户报告备案已提交材料，但审核结果未在本轮验证。官方CloudBase文档支持个人版云托管固定出口；企微官方消息接口正文尚未成功读取，设计明确要求实现前核对官方契约、否则保持模拟模式。文档自查已核对范围、矛盾、占位内容和敏感信息；`git diff --check` 与项目记忆验证器通过。本轮仅改设计及STATUS，没有修改产品代码，因此未重跑产品测试。下一步用户审阅该设计稿，通过后再编写实施计划；原 `project.config.json`、输出文件与缓存继续保留且排除本次文档提交。
