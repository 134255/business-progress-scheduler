...
- The protected account-aware business list does not yet return a node-assignment aggregate, so the dashboard can show newly created lines and active/completed counts but reports `pendingMine` as zero. A later feedback/dashboard task must add a protected assignment summary before that statistic is authoritative.
...
1. Execute Task 7 from `docs/superpowers/plans/2026-08-07-template-node-fields.md`: register and authorize image, PDF, and video evidence.
2. Continue the remaining feedback, rejection, freeze, amendment, and evidence-retention tasks in plan order from the isolated `codex/` worktree.
