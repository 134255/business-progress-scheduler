...
- 2026-08-26 项目所有者已确认“当前节点文本智能识别与字段预填”完整设计并批准直接进入实施，书面规格为 `docs/superpowers/specs/2026-08-26-node-text-smart-fill-design.md`，架构决策为 `docs/memory/decisions/ADR-0013-ai-assisted-current-node-text-fill.md`，逐项 TDD 计划为 `docs/superpowers/plans/2026-08-26-node-text-smart-fill.md`。活动处理人可在当前可编辑节点主动粘贴最多 8,000 字符文本，服务端只把该原文和当前节点规范化字段定义交给 CloudBase AI+ 内置托管模型；账号、售后元数据、其他节点、反馈、审核和凭证不会发送。识别只返回预览候选，已有值默认不覆盖，用户确认后也只写页面表单，仍须经过既有保存或提交。单选和多选只匹配既有选项，精确匹配优先，模糊语义匹配置信度至少 0.5。独立 `nodeTextParser` 只能消费 `businessApi` 签发的五分钟一次性内部票据；票据绑定账号、售后、节点、版本、字段定义摘要和原文摘要，不保存原文或结果，并以每分钟 10 次、每个上海自然日默认 300 次及同账号单飞限制控制费用。每日额度仅允许服务端通过 `NODE_TEXT_PARSE_DAILY_LIMIT` 在 `1..1000` 范围内调整，缺失使用 300，损坏配置失败关闭。当前生产代码、自动化 RED/GREEN、`nodeTextParser`、两个新集合、索引、CloudBase AI+ 配置、部署和真机验收全部为 `unverified`。下一步是按实施计划从解析协议 RED 开始开发；操作员自己的 `project.config.json` 修改继续保持未读取、未修改、未暂存、未提交。
