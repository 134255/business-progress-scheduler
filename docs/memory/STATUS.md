...
- Task 8-R is an explicitly approved follow-up after Task 8 exhausted its five formal-review fix rounds. The written design is approved and the implementation plan is ready. The design removes the remaining actor-facing recovery race by starting every actor-facing `reserved -> aborting` transition atomically in a transaction that reloads and authorizes the current actor, line, node, and exact reservation. Actorless chunked rollback may only continue an already-`aborting` reservation; the repository-only Task 11 maintenance entry may independently start expired recovery but must remain absent from public services and routes. Production code remains unchanged pending execution-mode selection.
...
1. Execute `docs/superpowers/plans/2026-08-10-task-8r-atomic-feedback-recovery.md` using deterministic RED/GREEN concurrency coverage and a fresh Task 8-R formal-review ledger.
