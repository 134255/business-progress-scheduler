...
- 2026-08-12 节点独立审核流程 Task 10 已完成本地实现与回归。新增可独立部署的 `workflowReminder` 云函数；计划入口只信任平台 `getWXContext().TRIGGER_SRC === 'timer'` 并使用服务端时钟，忽略事件载荷中的类型、时间和批量。工作时间固定为上海 09:00—20:00、无午休，日历缺失或损坏时失败关闭；处理与审核候选每周期合计不超过 40，并分别用安全扫描游标推进。通知编号分别由节点编号与累计工作小时，以及审核轮次、审核账号与累计工作小时确定性生成；通知和下一小时游标在固定文档事务中原子写入，事务重新读取活动业务、当前节点、轮次、内部账号、投票和既有通知。会签只提醒未投票审核人，或签已结束、任一驳回、停用账号、终态业务、换轮次和 `pending_calendar` 均停止小时提醒，既有管理员日历告警保持不变；通知正文仅含允许的类型、收件账号编号、业务/节点/轮次编号、累计小时、状态和服务端时间，不保存业务字段、凭证、OpenID、请求摘要或预约信息。TDD 初始 RED 为 4 个测试文件全部因生产模块不存在而预期失败；补强的旧游标、当前节点、双轮次和组合筛选测试也先失败再转绿。最终本地回归为 `workflowReminder` 20/20、`businessApi` 473/473、`calendarSync` 47/47、`evidenceRetention` 19/19、小程序 131/131、WXML 4/4；变更 JavaScript 语法、锁文件离线一致性和 `git diff --check` 通过。真实 CloudBase 部署、Timer 来源形状、复合索引、权限、并发与触发器仍未验证；触发器和 Task 11 部署资料均未修改。
